/**
 * Network Add Command Handler
 * Handles adding a custom network configuration using the Core API
 */
...
export async function addHandler(args: CommandHandlerArgs) {
  const { logger } = args;
  ...
  try {
    ...
    const confirm = await prompt<ConfirmSave>({
      type: 'confirm',
      name: 'ok',
      message: 'Save this network configuration even if health checks failed?',
    ...
    logger.error(formatError('❌ Failed to add network', error));
    process.exit(1);
  }
}
